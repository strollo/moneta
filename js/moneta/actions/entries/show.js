...
function showDetails(accountGroup, activityType, accountID, _title) {
	...
	// The target URL
	var url = moneta.Globals.data.DATA_ENTRY_DETAILS + '?node=root' + (accountGroup ? ('&acct_group=' + accountGroup) : '') + (activityType ? ('&type=' + activityType) : '') + (accountID ? ('&account=' + accountID) : '');
	// The store
